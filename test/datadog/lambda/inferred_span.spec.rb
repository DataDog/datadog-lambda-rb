...
describe Datadog::Lambda::InferredSpan do
  ...
  describe '.finish' do
    subject(:finish) { described_class.finish(span, response) }

    after { span&.finish unless span&.finished? }

    let(:span) { Datadog::Tracing.trace('test.inferred') }
    let(:response) { {'statusCode' => 200} }

    context 'when span is nil' do
      let(:span) { nil }

      it { expect(finish).to be_nil }
    end

    context 'when response contains statusCode' do
      it 'tags status and finishes the span' do
        finish

        aggregate_failures('status and lifecycle') do
    ...
    context 'when response is not a Hash' do
      let(:response) { 'not a hash' }

      it 'finishes without status tag' do
        finish

        aggregate_failures('no status, still finished') do
    ...
    context 'when an error occurs' do
      before do
        ...
        allow(span).to receive(:finish).and_raise(StandardError, 'boom')
      end

      let(:span) { double('span', finished?: true) }
