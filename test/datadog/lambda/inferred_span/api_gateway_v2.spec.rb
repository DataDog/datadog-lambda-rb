...
RSpec.describe Datadog::Lambda::InferredSpan::ApiGatewayV2 do
  ...
  context 'when all fields are present' do
    it { expect(parser.span_name).to eq('aws.httpapi') }
    ...
    it { expect(parser.api_id).to eq('xyz789') }
    it { expect(parser.stage).to eq('prod') }
    it { expect(parser.request_time_ms).to eq(1_700_000_000_000) }
    it { expect(parser.user_agent).to eq('TestAgent/2.0') }
    it { expect(parser.arn_path_prefix).to eq('apis') }
  end
  ...
  context 'when routeKey has no method prefix' do
    ...
    it { expect(parser.resource_path).to eq('$default') }
  end
  ...
  context 'when optional fields are missing' do
    ...
    it { expect(parser.method).to be_nil }
  end
end
