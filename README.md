...
1. If you are using the gem package `datadog-lambda-rb`, upgrade it to at least version `v0.6.0`. You also need to add the latest version of the datadog tracer `ddtrace` as a dependency. Keep in mind that ddtrace uses native extensions, which must be compiled for Amazon Linux before being packaged and uploaded to lambda. For this reason we recommend using the lambda layer.
